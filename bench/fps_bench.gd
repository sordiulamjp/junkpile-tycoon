...
## VR-04：幀數實測工具（唔隨貨出街）——S8+ 上 100／200／300 個碎片剛體
## 嘅平均 fps，用嚟定 GameConstants.debris_rigidbody_cap／
## frenzy_debris_degrade_steps。跟 `ads/ad_test.tscn` 同一個做法：
## 唔搶 `run/main_scene`，出 APK 前臨時將 project.godot 嘅
## run/main_scene 改去 res://bench/fps_bench.tscn，裝機跑完之後改返。
##
## 三個 phase 逐級加碼片數（唔清舊嘅，直接疊加到下一個目標數），
## 每個 phase 跳過首 1 秒（啱啱生完嗰下正常會跌幀，唔計落平均），
## 之後 5 秒取樣。結果 print()（adb logcat 睇到）+ 寫落
## user://fps_bench_report.txt（adb pull 攞）+ 畫面 Label 顯示到跑完
## 唔會自動熄，方便截圖存證。

const PHASES: Array[int] = [100, 200, 300]
...
func _process(delta: float) -> void:
	...
	_label.text = "Phase %d/%d: %d rigidbodies\nt=%.1fs  fps=%.0f" \
		% [_phase_idx + 1, PHASES.size(), PHASES[_phase_idx], _phase_elapsed, fps]
